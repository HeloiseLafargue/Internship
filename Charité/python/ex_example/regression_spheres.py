"""
1st version : spheres with deformetrica
"""
...
data_path = '/home/icm-admin/Documents/Internship/data/'

data_base = os.path.join(data_path, 'spheres/data/')  # clean concatenation of 2 pathways 

stl_files = glob.glob(data_base + "*.stl")  # list of the files' pathways

figsize = 5
f = plt.figure(figsize=(len(stl_files) * figsize, 1 * figsize))

for i, stl_file in enumerate(stl_files):  
    ax = f.add_subplot(1, len(stl_files), i+1, projection='3d')
    
    #vtk_file = vtk_files[0]
    m = re.findall(r".*\/(.*)\..*", stl_file)  # list which contains the name  of the considered file
    target_id = m[0]
    target_points__raw, _, target_connectivity__raw = dfca.io.DeformableObjectReader.read_file(stl_file, extract_connectivity=True)
...
data_path = '/home/icm-admin/Documents/Internship/data/'
data_base = os.path.join(data_path, 'spheres/data/')
...
dataset_specifications = {
    'dataset_filenames': [
        [{'sphere': os.path.join(data_base, 'Sphere_Small.stl')}]],
    'subject_ids': ['subj1']
}

template_specifications = {
    'sphere': {'deformable_object_type': 'surfacemesh',
                'kernel_type': 'keops', 'kernel_width': 11,
              'noise_std': 0.1,
              'filename': os.path.join(data_base, 'Sphere_Large.stl'),
...
path_to_template__ini = template_specifications['sphere']['filename']
...
for i, momentum__est in enumerate(momenta__est): 
    ...
    path_to_target__raw = dataset_specifications['dataset_filenames'][i][0]['sphere']
